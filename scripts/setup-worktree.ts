...
function main(): void {
  ...
  writeFileSync(
    ...
    [
      ...
      "# lives only in the main checkout. Every checkout's dev server syncs into it, and",
      "# the scraper's CLI reads this file to do the same, one at a time under a shared",
      "# lock, so sharing it is safe. notes.db beside it is shared on purpose, so notes",
      "# outlive this worktree. Delete this file once you have scraped a database here,",
      "# and notes will then be written beside that one instead.",
